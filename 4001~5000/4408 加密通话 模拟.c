...
char key[114514], ori[114514], ans[114514];
char tmp[114514], tmp2[114514];
...
char judge_col(int y)
{
	...
	for (; x < 26; ++x)
	{
		...
		if (occur[sheet[x][y] - 'A']) return 0;
		else occur[sheet[x][y] - 'A'] = 1;
	}
	return 1;
}
...
{
	int y = 0;
	for (; y < 26; ++y)if (!judge_col(y)) return 0;
	return 1;
}

int main()
{
	while (fgets(key, 114514, stdin))
	{
		flag = 1;
		...
		fgets(tmp, 114514, stdin), crlf(tmp, &len_tmp);
		copy_without_space(ori, &len_ori, tmp, &len_tmp);
		fgets(tmp2, 114514, stdin), crlf(tmp2, &len_tmp2);
