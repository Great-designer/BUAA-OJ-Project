...
struct dis_info
{
    long long dis;
    int label;
    dis_info(long long d, int no) : dis(d), label(no)
    {
    }
...
namespace static_KD_Tree
{
    ...
    dis_info get_dis(const point &from, const point &to)
    {
        return dis_info(dis(from, to), to.label);
    }
    ...
    void dfs(dis_info &cur_min_dis, int l, int r, const point &a)
    {
        ...
        long long d = div[mi] ? (a.x - p[mi].x) : (a.y - p[mi].y);
    ...
    {
        dis_info ret = dis_info(1145141919810114514ll, 0);
        dfs(ret, 1, n, a);
        return ret;
    }
}
...
int main()
{
    ...
    while (T--)
    {
        ...
        long long ans = 1145141919810114514ll;
        while (N--)
        {
            query_point.input(0);
