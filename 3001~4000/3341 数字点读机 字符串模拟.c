...
long long n;
long long splitPart[4];
int printPart[4];
int printsize;

void printSingleNum(long long num)
{
	//2位数
	long long ten = num / 10;
	long long one = num % 10;
	if (ten == 0)
	{
		if (one == 1)printf("one");
		if (one == 2)printf("two");
...
void print(long long num, int part)
{
	long long hundred = num / 100;
	long long tenandone = num % 100;
	if (hundred > 0)
	{
		printSingleNum(hundred);
		printf(" hundred");
		if (tenandone > 0)
