...
struct RBNode//红黑树节点
{
	...
	RBNode(T v = T(), bool RB = 1, RBNode* f = NULL,RBNode* l = NULL, RBNode* r = NULL, int s = 1): val(v), color(RB), ftr(f), lc(l), rc(r), _size(s) {}
	//删除节点专用后继
	RBNode* single_succ()
	{
		RBNode* ret = rc;
...
struct RedBlackTree
{
	...
	void connect34(RBNode<T>* nroot, RBNode<T>* nlc, RBNode<T>* nrc,RBNode<T>* ntree1, RBNode<T>* ntree2, RBNode<T>* ntree3, RBNode<T>* ntree4)
	{
		nlc->lc = ntree1;
	...
	//不允许重复的查找,只要找到对应值就收手
	RBNode<T>* rfind(T v, const int op)
	{
		RBNode<T>* ptn = _root;
		_hot = NULL;
		while (ptn && ptn->val != v)
	...
	void SolveDoubleRed(RBNode<T>* nn)
	{
		while ((!(nn->ftr)) || nn->ftr->color == 1)
		{
			...
			RBNode<T>* p = nn->ftr;
			if (p->color == 0)return;//case 1:没有双红,直接返回
			RBNode<T>* u = bro(p);
			RBNode<T>* g = p->ftr;
			//case 2:RR-2
			if (u != NULL && u->color == 1)
	...
	//如果是根节点了,就可以直接停机了
	void SolveDoubleBlack(RBNode<T>* nn)
	{
		while (nn != _root)
		{
			RBNode<T>* p = nn->ftr;
			RBNode<T>* b = bro(nn);
			if (b->color == 1)   //Case 1:BB-3
			{
				b->color = 0;
				p->color = 1;
				if (_root == p)_root = b;
			...
			if (b->lc && b->lc->color == 1)   //Case 2-1:BB-1
			{
			...
			else if (b->rc && b->rc->color == 1)   //Case 2-2:BB-1
			{
				bool old_p_color = p->color;
				p->color = 0;
				if (p->lc == nn)  //zag
	...
	void checkconnect(RBNode<T>* ptn)
	{
		if (!ptn) return;
	...
	iterator<T> Kth(int Rank)
	{
		return iterator<T>(findKth(Rank, _root));
	}
