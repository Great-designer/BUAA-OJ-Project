...
double single_magic[510];//单次法术第i次操作的概率
double spin[510][510], magic[510][510];
double dp[510][510];//dp[i][j]表示还剩下i轮,还有j个箱子没开的时候的期望
...
int main()
{
	...
	{
		init();
		n = read(), m = read(), t = read(), k = read(), p = (double)read() / 100.0;
