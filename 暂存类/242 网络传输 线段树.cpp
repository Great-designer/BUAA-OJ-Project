...
struct SegmentTree
{
	...
	void pushup(int root)
	{
		...
		nodes[root].value_max=(nodes[Lchild(root)].value_max)>(nodes[Rchild(root)].value_max)?(nodes[Lchild(root)].value_max):(nodes[Rchild(root)].value_max);
	}
	...
	int queryMax(int root, int l, int r, int L, int R)
	{
		...
		if (R > m)ret=ret>(queryMax(Rchild(root), m + 1, r, L, R))?ret:(queryMax(Rchild(root), m + 1, r, L, R));
		return ret;
	}
};
