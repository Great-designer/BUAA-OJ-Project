...
void update(double newT)   //执行指令之前,更新角色属性至newT
{
...
void handleEquip(double a, double b)  //装上防护甲
{
	equipped = true;
	singleRate = a / 100.0;
	remainPower = b;
}
void handleEat(double a, double b, double c)
{
	hunger += a, san += b, hp += c;
...
void handleSan(double a)
{
	sanBuff += a;
}
void handleHP(double a)
{
	hpBuff += a;
}
void query(int n)
