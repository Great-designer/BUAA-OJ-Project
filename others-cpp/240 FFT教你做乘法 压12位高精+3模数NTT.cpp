...
namespace Arbitrary_NTT_Solver
{
	...
	const __int128_t all_mod = (__int128_t)1004535809 * (__int128_t)1007681537 * (__int128_t)1045430273;
	const __int128_t MP[3] = {all_mod + (__int128_t)1053460784322969601ll * (__int128_t)(-480795620), all_mod + (__int128_t)1050172145041145857ll * (__int128_t)(-401215089), all_mod + (__int128_t)1012252187984658433ll * (__int128_t)(-128816555)};
	struct MOD3
	{
		int reminder[3];
...
//			assert(!((M[i] - 1) & ((1 << L) - 1)));
	...
	void mul(long long a[], int n, long long b[], int m, long long res[], long long mod, int *res_len)
	{
		// multiple case
//		assert((*res_len) >= n + m + 1);
		...
		if (n < 100 / (m + 1) || n < 3 || m < 3)
			for (int i = 0; i <= n; ++i)
				...
				{
					// res[i + j] += a[i] * b[j];
					__int128_t x = (__int128_t)res[i + j] + (__int128_t)a[i] * (__int128_t)b[j];
					res[i + j] = 0;
					int offset = 0;
					while (x >= mod)
...
//			assert(initialized);
			...
			for (int i = 0; i <= n + m; ++i)
			{
				__int128_t x = (__int128_t)res[i] + mod_a[i].val();
				res[i] = 0;
				int offset = 0;
				while (x >= mod)
...
struct BigNumber
{
	const long long digit[13] = {1ll, 8ll, 64ll, 512ll, 4096ll, 32768ll, 262144ll, 2097152ll, 16777216ll, 134217728ll, 1073741824ll, 8589934592ll, 68719476736ll}, MODD = 12;
	static const long long MOD = 68719476736ll, base_radix = 8;
	...
	{
		len = p.len, f = p.f, a = new long long[len];
		memcpy(a, p.a, len * sizeof(long long));
	}
